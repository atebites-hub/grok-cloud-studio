...
function refuseLiveNameTwin(id: string, name: string, runStatus: string): never {
  process.stdout.write(`CLOUD_LAUNCH_ERR id=${id} name=${name} runStatus=${runStatus}\n`);
  console.error(
...
/** LIV-59: refuse create when a live Extra High already has this --name. Never Bot CloudAgent. */
