#!/usr/bin/env bash
# Run Grok Cloud Studio @cursor/sdk CLIs: launch|list|occupancy|status|watch|followup|result|wait-notify
...
case "$CMD" in
  ...
  -h|--help|help|"")
    echo "usage: run.sh <launch|list|occupancy|status|watch|followup|result|wait-notify> [args...]" >&2
    exit 2
    ;;
  *)
    echo "usage: run.sh <launch|list|occupancy|status|watch|followup|result|wait-notify> [args...]" >&2
