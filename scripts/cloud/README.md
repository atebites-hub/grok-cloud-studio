...
Cloud agents are durable membership. `GET /v1/agents` `status` stays `ACTIVE` until archive, even after the latest run is terminal. Directors who only look at `ACTIVE` treat leftover FINISHED grunts as spinning workers. Existence is not liveness.
...
Never Bot CloudAgent. Model stays grok-4.6 xhigh `fast=false`. Living Sky `LIV-*` (not Black Swan).
