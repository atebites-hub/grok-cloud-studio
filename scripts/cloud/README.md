...
| Script | Purpose |
|---|---|
| `../launch-cloud-extra-high.sh --name NAME "prompt"` | Create Extra High agent + initial run (PR auto). Prints `CLOUD_LAUNCH_OK`. **REFUSE** if a live `runStatus=RUNNING` agent already has that name (LIV-59, no twin remint). Leftover `ACTIVE`+`FINISHED` does not block. Never Bot CloudAgent. |
...
# 1) Launch grunt
#    LIV-59 --name REFUSE if a live runStatus=RUNNING Extra High already has that name
