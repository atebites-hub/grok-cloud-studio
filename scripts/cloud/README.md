...
Hard-wired Extra High create **and send** (SDK `Agent.create` + `sendPinned` / REST `POST /v1/agents` and `POST /v1/agents/{id}/runs`):
...
- REST follow-up posts the same model object
...
Never launch a Grok Bot CloudAgent. Studio Linear is Living Sky (`LIV`), never Black Swan. Do not vendor Hermes.
