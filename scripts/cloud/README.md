...
| `spawn-waiter.sh --id bc-… [--seat SEAT]` | Register ledger on the owning director seat (`GCS_DIRECTOR_SEAT`, e.g. `cloud`) + detached `wait-notify`. Do not silently default to `floor` when the owner is `cloud`. |
...
| `occupancy-count.sh` | Paginated occupancy catalog (`Agent.list` / `GET /v1/agents` via `nextCursor`, page size 100). Latest `runStatus` via listRuns (`GET /v1/agents/{id}/runs`). `CREATING` maps as `RUNNING`. Leftover `ACTIVE`+`FINISHED` is not a worker. Split Palemon vs GCS toward floor 8 (`palemon=` `gcs=` `cap=` `palemon_must=` `gcs_must=`). Prints `CLOUD_OCCUPANCY running= leftover_active= creating= listed= pages= palemon= gcs= cap= palemon_must= gcs_must=`. Fail-closed `CLOUD_OCCUPANCY_ERR` if a page or listRuns errors — never fake `running=0`. Distinct from #142 catalog paginate and HOLD #132 concurrency. |
...
| `result-cloud-agent.sh <bc-id>` | Non-blocking result/context JSON (`repoUrl` = bound `repos[0].url`). `prUrl` URL vs none: FINISHED + prUrl none stamps `directorAction=CLOSE` (leftover of merged shard is CLOSE; no MERGE_REQUEST, no twin Extra High). |
...
`scripts/cloud/occupancy-count.sh` is the occupancy path (SDK `occupancy.ts` / REST `occupancy_count.py`). Latest `runStatus` is **listRuns** (`Agent.listRuns` / `GET /v1/agents/{id}/runs` collection), not a pinned leftover `latestRunId`. **`CREATING` maps as `RUNNING`**. Leftover `ACTIVE`+`FINISHED` is not a worker. Split Palemon vs GCS toward floor 8 (`palemon=` `gcs=` `cap=` `palemon_must=` `gcs_must=`). It prints `CLOUD_OCCUPANCY running=N leftover_active=N creating=N listed=N pages=N palemon=N gcs=N cap=N palemon_must=N gcs_must=N`. If any catalog page or listRuns errors, it prints `CLOUD_OCCUPANCY_ERR` and exits non-zero — **never** a fake `running=0` from a partial list. Skip `GCS_BOT_AGENT_ID`. Distinct from leftover occupancy GCS #132 (do not rebase) and from #142 catalog `nextCursor` paginate.
