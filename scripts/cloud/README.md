...
`scripts/directors/fleet-shepherd.py` is an **orphan-only** safety net: it skips rows with a live `waiter_pid` or `notified_by` in `{waiter, webhook, shepherd}`. `fleet_ledger.notify_owner` is idempotent: a second notify on a row already `notified_by=waiter` does not A2A-ping again (waiter+shepherd must not double-fire `FLEET_DONE`).
