...
# Parse the API key from agent.env. Never `source` the file: `set -a; source`
...
cloud_read_api_key_from_file() {
  local env_file="$1" line value prefix="CURSOR_API_KEY"
  [[ -f "$env_file" ]] || return 1
...
cloud_load_auth() {
  ...
  # emptiness check prints the API key under `bash -x`. Export while +x
  # is still off so child name-twin / SDK processes inherit the key.
