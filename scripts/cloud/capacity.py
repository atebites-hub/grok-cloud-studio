"""Cursor Cloud runStatus vs agent status; playability/art must-launch.
...
If playability/art work is in progress and in-flight count for the target
repo is below GCS_CLOUD_RUNNING_CAP (default 8), the cloud mind MUST launch.
"""
...
DEFAULT_RUNNING_CAP = 8
IN_FLIGHT_RUN = frozenset({"RUNNING", "CREATING"})
LINEAR_WORKSPACE = "Living Sky"

_PLAY_ART_RE = re.compile(r"\bplayability\b|\bart\b", re.IGNORECASE)
_KV_RE = re.compile(r"(\w+)=(\S*)")


def running_cap(raw: str | None = None) -> int:
    text = (raw if raw is not None else os.environ.get("GCS_CLOUD_RUNNING_CAP") or "").strip()
    if not text:
        return DEFAULT_RUNNING_CAP
    try:
        value = int(text)
    except ValueError:
        return DEFAULT_RUNNING_CAP
    return value if value > 0 else DEFAULT_RUNNING_CAP
...
def work_is_playability_or_art(text: str) -> bool:
    return bool(_PLAY_ART_RE.search(text or ""))


def must_launch_cloud(
    *,
    work: str,
    running_count: int,
    cap: int | None = None,
) -> bool:
    limit = DEFAULT_RUNNING_CAP if cap is None else cap
    if cap is None:
        limit = running_cap()
    if limit <= 0:
        return False
    return work_is_playability_or_art(work) and running_count < limit
...
def must_launch_reason(*, work: str, running_count: int, cap: int) -> str:
    if not work_is_playability_or_art(work):
        return "not-playability-art"
    if running_count >= cap:
        return "at-cap"
    return "playability-art-below-cap"
...
def decide(
    *,
    ...
    limit = running_cap() if cap is None else cap
    must = must_launch_cloud(work=work, running_count=running_count, cap=limit)
    return {
        "running": running_count,
        "cap": limit,
        "repo": repo,
        "must_launch": must,
        "reason": must_launch_reason(work=work, running_count=running_count, cap=limit),
    }
...
def cmd_decide_from_list(*, work: str, repo: str, cap: int | None) -> int:
    text = sys.stdin.read()
    ...
    decision = decide(work=work, running_count=n, cap=cap, repo=repo)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "decide-from-list":
        return cmd_decide_from_list(work=args.work, repo=args.repo, cap=args.cap)
    return 2
