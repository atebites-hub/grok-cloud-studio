#!/usr/bin/env python3
"""Empty GitHub checks are not ship-gate evidence.

MERGEABLE (`mergeable_state=clean`) with `check_runs=[]` is leftover-green
theatre, not proof that `.venv/bin/pytest -q` and
`python3 scripts/secret_scan.py` ran. Unique ship-gate YAML is already on
origin/main. The required GitHub Actions check is named
"pytest -q and secret_scan".

Never prints GH_TOKEN / GITHUB_TOKEN / CURSOR_API_KEY.
Never vendors Hermes. Never merges GCS #26+#28.
"""
