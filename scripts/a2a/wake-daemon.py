#!/usr/bin/env python3
"""GROW wake: inbox.jsonl → local ACP session/prompt into live grok agent serve.

One `grok agent serve` per seat that NEVER exits, plus a local client that
turns inbox growth into ACP `session/prompt` INSIDE that same serve process.

Success is mail delivered (session/prompt accepted). Inject disconnects
without session/cancel so the agent keeps the turn. Advance wake.offset
on ACP_INJECT_OK / HANDOFF even if the model is still running.
...
def _write_grow_mode(seat: str) -> None:
    path = _seat_dir(seat) / "grow.mode"
    path.write_text(
...
def ensure_identity(seat: str, seat_dir: Path) -> tuple[Path, Path]:
    """Copy versioned SOUL.md + MEMORY.md into the seat state dir (serve reads them)."""
    src = SOULS_DIR / seat
    alias = SOULS_DIR / canonical_seat(seat, ROOT)
    seat_dir.mkdir(parents=True, exist_ok=True)
    grok_home = seat_dir / "grok-home"
    grok_home.mkdir(parents=True, exist_ok=True)
    install_grok_home_auth(grok_home)
    soul_dst = seat_dir / "SOUL.md"
    mem_dst = seat_dir / "MEMORY.md"
    soul_src = src / "SOUL.md"
    if not soul_src.is_file():
        soul_src = alias / "SOUL.md"
    mem_src = src / "MEMORY.md"
    if not mem_src.is_file():
        mem_src = alias / "MEMORY.md"
    if soul_src.is_file():
        soul_dst.write_text(soul_src.read_text(encoding="utf-8"), encoding="utf-8")
    elif not soul_dst.is_file():
        soul_dst.write_text(
            f"# {seat}\n\nNamed identity for Grok Cloud Studio seat `{seat}`.\n",
            encoding="utf-8",
        )
    if mem_src.is_file():
        body = mem_src.read_text(encoding="utf-8")
        mem_dst.write_text(body, encoding="utf-8")
        (grok_home / "memory.md").write_text(body, encoding="utf-8")
    elif not mem_dst.is_file():
        mem_dst.write_text(f"# Memory — {seat}\n", encoding="utf-8")
    return soul_dst, mem_dst
...
def ensure_seat_serve(seat: str) -> int:
    ...
    if proc.stdout:
        ...
        if "server-key=" in out:
            out = out.split("url=", 1)[0].rstrip() + " url=ws://127.0.0.1:(redacted)"
...
def _compose_prompt(seat: str, rec: dict[str, Any], text: str) -> str:
    ...
    if disp._is_cloud_launch_message(text, parts):
        return disp._compose_director_work(task_id, context_id, text)
...
def process_once(seat: str, *, dry_run: bool = False) -> dict[str, Any]:
    ...
    for end_offset, rec in records:
        ...
        extra_env["GCS_A2A_CONTEXT"] = context_id
        extra_env["GCS_A2A_SEAT"] = seat
        extra_env["GROK_MEMORY"] = "1"
...
def run_forever(seat: str) -> int:
    ...
    _write_pid(seat)
    _write_grow_mode(seat)
    ensure_identity(seat, _seat_dir(seat))
    print(
