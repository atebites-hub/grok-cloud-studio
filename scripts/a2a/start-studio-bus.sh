...
# standby otherwise). A live bot-bridge.pid is never killed or reminted on
# start / recover. Seat ACP daemons are opt-in:
...
# start does not spawn bot-bridge unless GCS_BOT_BRIDGE=1.
...
start            hub + dispatch + fleet-shepherd (idempotent; recycle leftover
                 ...
                 A live bot-bridge.pid is left running.
