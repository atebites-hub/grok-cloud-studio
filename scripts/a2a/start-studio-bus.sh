...
BOT_BRIDGE_STANDBY_FILE="$STATE_DIR/bot-bridge.standby"
...
# Hive stale membership: write bot-bridge.standby so a later host start
# (watchdog) does not resurrect from an evicted ghost pidfile.
mark_bot_bridge_standby() {
  local pid="${1:-none}"
  printf 'pid=%s\n' "$pid" >"$BOT_BRIDGE_STANDBY_FILE"
}

evict_stale_bot_bridge() {
  local pid="${1:-none}"
  rm -f "$BOT_BRIDGE_PID_FILE"
  mark_bot_bridge_standby "$pid"
  echo "STUDIO_BUS_BOT_BRIDGE_STALE pid=$pid (pidfile is not liveness; not starting)"
}
...
case "$cmd" in
  start)
    ...
    if pid_alive "$bridge_pid"; then
      ...
      rm -f "$BOT_BRIDGE_STANDBY_FILE"
    elif [[ -f "$BOT_BRIDGE_PID_FILE" || -f "$BOT_BRIDGE_STANDBY_FILE" ]]; then
      evict_stale_bot_bridge "${bridge_pid:-none}"
      bridge_pid=""
    else
      rm -f "$BOT_BRIDGE_PID_FILE"
  ...
  stop)
    ...
    rm -f "$BOT_BRIDGE_PID_FILE" "$BOT_BRIDGE_STANDBY_FILE"
    if pid_alive "$shep_pid"; then
  ...
  status)
    ...
    if pid_alive "$bridge_pid"; then
      ...
      evict_stale_bot_bridge "${bridge_pid:-none}"
      bridge_pid=""
    elif [[ -f "$BOT_BRIDGE_STANDBY_FILE" ]]; then
      bridge_pid=""
    fi
    daemon_flag="off"
