#!/usr/bin/env bash
# Register tcarac/taskboard stdio MCP, Living Sky Linear HTTP, and
# chrome-devtools (Grok catalog browser MCP) into each seat GROK_HOME/config.toml.
...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  ...
  echo "Register taskboard + Linear HTTP + chrome-devtools stdio MCP into seat GROK_HOME/config.toml." >&2
