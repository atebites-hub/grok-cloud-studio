...
are dropped (fail closed). Stdlib only. Never prints API keys.
"""
    ...
    if isinstance(repos, list):
        for item in repos:
            if isinstance(item, str) and item.strip():
                ...
                found = str(item.get("url") or item.get("repository") or item.get("repo") or "").strip()
                if found:
                    urls.append(found)
    source = payload.get("source")
    if isinstance(source, dict):
        found = str(source.get("repository") or source.get("url") or source.get("repoUrl") or "").strip()
        if found:
            urls.append(found)
    git = payload.get("git")
