...
PLUGINS: dict[str, Plugin] = {
    ...
    "liv_stamp": Plugin(schema=LIV_STAMP_SCHEMA, call=plugin_liv_stamp),
}
...
def process_once(seat: str, *, runner: Callable[..., Any] | None = None) -> dict[str, Any]:
    ...
    for end_offset, rec in records:
        ...
        liv_id = ""
        try:
            stamped = maybe_stamp_after_task(
                seat=seat,
                task_id=task_id,
                evidence=assistant_text,
            )
            if stamped.get("ok"):
                liv_id = str(stamped.get("issue") or "")
        except Exception as exc:
            print(
                f"LIV_STAMP_SKIP reason=hook-fail {redact(str(exc))}",
                file=sys.stderr,
                flush=True,
            )
        result: dict[str, Any] = {
