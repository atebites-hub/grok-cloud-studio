"""status.sh leftover ACTIVE+FINISHED vs RUNNING; 404 run GET is none.

Batch status --ids + runStatus compact already landed on main (#60).
list.sh / sdk/list.ts / MCP cloud_list already print runStatus.
Unique remaining vs origin/main: pytest that REST status (via
status_fetch.py) still prints runStatus=FINISHED vs RUNNING, treats a
missing latestRunId or HTTP 404 run GET as runStatus=none, and unwraps
{agent:}/{run:} envelopes. Does not remint --ids, occupancy floor, or
cloud_list. Does not vendor Hermes. Never Bot CloudAgent. Floor parked.
"""
...
def membership_is_active(fields: dict[str, str]) -> bool:
    """Main compact uses agentStatus=; list-row format uses status=."""
    return fields.get("agentStatus") == "ACTIVE" or fields.get("status") == "ACTIVE"
...
def test_status_sh_leftover_active_finished_prints_run_status_finished(
    ...
    assert membership_is_active(fields)
...
def test_sdk_list_ts_already_prints_run_status_on_main() -> None:
    """Do not remint list.ts / cloud_list / status --ids occupancy batch."""
    ...
    assert "--ids" in status_sh
    assert "status_fetch.py" in status_sh
