Feature: Grok-bot-like mind plugins (ticket/A2A/cloud) without Hermes (LIV-63 remaining)
  ...
  and #76 (mailbox disk turn + spawn PATH + ticker). Grok Build minds get
  grok-bot-like plugins for ticket, A2A, and cloud — grok `plugin.json` MCP.
  Do not vendor NousResearch/hermes-agent. Not Hermes `plugin.yaml`.
