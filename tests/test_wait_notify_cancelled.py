...
Does not remint occupancy. GCS #52 draft lookup and GCS #58
mergeable=CONFLICTING HOLD squash both remain. Never Bot CloudAgent.
Extra High stays grok-4.6 xhigh fast=false. Living Sky Linear (LIV).
"""
...
def test_footer_cancelled_inspect_not_bot_cloudagent() -> None:
    ...
    # Occupancy HOLD. Keep GCS #52 draft lookup and #58 mergeable HOLD.
    assert "githubPrIsDraft" in src
    assert "pr-draft.ts" in src
