...
runStatus on the same line as id=. Does not remint list.sh (runStatus
already on main). Never Bot CloudAgent. Model remains grok-4.6 xhigh
fast=false.
"""
...
def test_list_is_not_reminted_by_batch_status() -> None:
    """Capacity unique remaining is status --ids; list.sh already has runStatus."""
    list_ts = LIST_TS.read_text(encoding="utf-8")
    list_sh = LIST_SH.read_text(encoding="utf-8")
    status_sh = STATUS.read_text(encoding="utf-8")
    status_ts = STATUS_TS.read_text(encoding="utf-8")
    fold_list = f"{list_sh}\n{list_ts}"
    assert "runStatus" in fold_list
    assert "--ids" not in list_sh
    assert "--ids" not in list_ts
    assert "--ids" in status_sh
    assert "--ids" in status_ts
    assert "Promise.all" in status_ts or "allSettled" in status_ts
