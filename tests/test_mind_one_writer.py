...
def test_process_once_is_sole_mail_txt_writer() -> None:
    ...
    bot_like = (Path(__file__).resolve().parents[1] / "scripts" / "a2a" / "mind_bot_like.py")
    harvest = bot_like.read_text(encoding="utf-8")
    assert 'mail.txt").write_text' not in harvest
    assert "(mind / \"mail.txt\").write_text" not in harvest
