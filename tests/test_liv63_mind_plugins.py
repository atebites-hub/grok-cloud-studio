"""LIV-63 remaining: grok-bot-like mind plugins (ticket/A2A/cloud).

Executable binding for tests/features/liv63_mind_plugins.feature.

Unique remaining vs origin/main after GCS #47 OPEN CONFLICTING DIRTY
(do not merge it) and unique LIV-63 #76 on main (mailbox disk turn + Extra
High spawn PATH). Remaining is grok `plugin.json` for ticket / A2A / cloud
without vendoring NousResearch/hermes-agent.

Mail-is-a-turn stays grok mailbox + pin + stay-up, not ACP overlay.
Does not land harvest #26/#28. Does not restack #47 cloud_list /
cloud_followup into mind.py. Extra High cloud_list stays on the
cursor-cloud MCP plane already on main. Never Bot CloudAgent.
...
def test_scenario_copied_plugins_honor_gcs_root(tmp_path: Path) -> None:
    ...
    # Extra High cloud_list is already the cursor-cloud plane on main (LIV-67).
    # Do not restack it (or cloud_followup) into mind.py helpers.
    assert "cloud_followup" not in cloud_names
    assert "cloud_followup" not in a2a_names
...
def test_studio_mind_still_exposes_ticket_without_restacking_pr47() -> None:
    ...
    for restack in PR47_MIND_RESTACK:
...
def test_scenario_mail_is_a_turn_stays_grok_mailbox() -> None:
    ...
    # Law text on origin/main: Extra High only. Do not treat the
    # prohibition string as a spawn path.
...
def test_docs_name_remaining_grok_bot_like_plugins() -> None:
    ...
    assert "install_mind_grok_plugins" in blob or "grok plugin install" in low
