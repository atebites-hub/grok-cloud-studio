...
check_runs=[]. That is not pytest -q + secret_scan. GCS #92 SUCCESS
is the required GitHub Actions job; this PR adds that gate plus
fail-closed collect/waiter HOLD. Do not remint runStatus list PRs.
...
def test_feature_file_states_liv94_law() -> None:
    ...
    assert "HOLD" in text
    assert "GCS #92 SUCCESS" in text or "ship-gate.yml" in text
...
def test_this_change_adds_the_gcs_92_success_ship_gate_workflow() -> None:
    """GCS #92 SUCCESS: job named pytest -q and secret_scan, fetch-depth 0."""
