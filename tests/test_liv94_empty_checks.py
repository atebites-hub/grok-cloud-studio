"""LIV-94 remaining: empty GitHub checks are not merge evidence.

Unique GitHub Actions ship-gate (pytest -q AND secret_scan) is already
on origin/main. MERGEABLE (mergeable_state=clean) with check_runs=[] is
leftover-green theatre, not proof the required check ran. This change
fail-closes collect / waiter HOLD. Do not remint ship-gate.yml. Do not
vendor Hermes. Do not merge GCS #26+#28. Never Bot CloudAgent. Never
print GH_TOKEN / GITHUB_TOKEN / CURSOR_API_KEY.
"""
...
def _empty_mergeable(*, pr: str = GCS41) -> ShipGateSnapshot:
    """Observed leftover-green theatre: MERGEABLE, no checks."""
...
def _ship_gate_ok_snapshot() -> ShipGateSnapshot:
    """Required GitHub Actions check named pytest -q and secret_scan."""
    return ShipGateSnapshot(
        pr_url=GCS62,
        mergeable_state="clean",
...
def test_ship_gate_example_is_pull_request_pytest_and_secret_scan() -> None:
    """Required check already on main — do not remint a second YAML."""
...
def test_unique_ship_gate_already_on_main_is_not_reminted() -> None:
    """Unique remaining vs origin/main is required-check evidence, not a second YAML."""
    wf = ROOT / ".github" / "workflows" / "ship-gate.yml"
    assert wf.is_file()
    text = wf.read_text(encoding="utf-8")
    assert "pytest -q and secret_scan" in text
    assert "scripts/ci/ship-gate.sh" in text
    assert "fetch-depth: 0" in text or "fetch-depth:\n          0" in text
    extras = sorted(
        p.name
        for p in (ROOT / ".github" / "workflows").iterdir()
        if p.suffix in {".yml", ".yaml"} and p.name != "ship-gate.yml"
    )
...
def test_collect_and_result_source_attach_ship_gate() -> None:
    """Director collect JSON, not only waiter A2A ping."""
...
def test_result_cloud_agent_rest_flags_empty_github_checks(
    ...
    """Director collect on MERGEABLE leftover-green theatre must print emptyChecks."""
    monkeypatch.setenv("GCS_ROOT", str(ROOT))
...
def test_do_not_vendor_hermes_or_merge_harvest() -> None:
    assert not (ROOT / "vendor" / "hermes-agent").exists()
    assert not (ROOT / "vendor" / "hermes").exists()
    evidence = ROOT / "scripts" / "cloud" / "ship_gate_evidence.py"
    helper = PR_CHECKS_TS.read_text(encoding="utf-8") if PR_CHECKS_TS.is_file() else ""
    blob = evidence.read_text(encoding="utf-8") if evidence.is_file() else ""
    blob += helper
    assert "hermes-agent" not in blob.lower()
    assert "message_agent" not in blob
    for marker in ("format_mail_turn", "filter_inbound_mail", "MAIL_MAX_CHARS"):
