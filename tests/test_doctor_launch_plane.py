...
def test_doctor_source_fail_closed_launch_plane_no_secrets_no_spawn() -> None:
    ...
    start = text.find("Cursor Cloud launch-plane")
    host = text.find("if command -v grok")
    block = text[start:host] if start != -1 and host != -1 else ""
    assert block, "missing Cursor Cloud launch-plane block"
    assert "remint" not in block.lower()
    assert "recreate cloud-env" in block.lower()
