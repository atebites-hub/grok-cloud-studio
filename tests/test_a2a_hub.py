...
def test_send_donald_capacity_ack_lands_on_orchestrator(hub: dict) -> None:
    ...
    proc = _send(hub, "--from", "floor", "donald", "CAPACITY_ACK ready=1")
    ...
    assert "TASK_STATE_SUBMITTED" in proc.stdout
    assert "TASK_STATE_COMPLETED" not in combined
...
def test_hub_post_donald_rewrites_to_orchestrator_inbox(hub: dict) -> None:
    ...
    assert (task.get("status") or {}).get("state") == "TASK_STATE_SUBMITTED"
    ack = ((task.get("status") or {}).get("message") or {}).get("parts")
...
def test_send_unknown_seat_still_404(hub: dict) -> None:
    proc = _send(hub, "not-a-seat", "should fail")
...
def test_bot_seats_are_not_cloud_or_acp_launch_targets() -> None:
    ...
    assert "never Bot CloudAgent" in launcher
    denied = subprocess.run(
        ["python3", str(LIB), "cloudagent-ok", "donald"],
        cwd=str(ROOT),
        capture_output=True,
        text=True,
        timeout=10,
