...
Disaster recovery: `./setup.sh` (deploy) and `./cleanup.sh` (teardown). Live DR loop: `./health_check.sh` + `./recover.sh`. Hive law apply-log: **[docs/studio/HIVE.md](docs/studio/HIVE.md)**. See **[docs/studio/WIPE.md](docs/studio/WIPE.md)**. Clone with `--recurse-submodules` (or `git submodule update --init --recursive`) so `vendor/taskboard` is the v0.6.0 source pin.
