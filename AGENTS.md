...
- Ship gate: `.venv/bin/pytest -q` and `python3 scripts/secret_scan.py`. Empty GitHub checks are not merge; `.github/workflows/ship-gate.yml` runs `scripts/ci/ship-gate.sh` as the check named **pytest -q and secret_scan**.
