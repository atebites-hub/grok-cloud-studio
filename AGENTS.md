...
- Opt-in Grok Build mind (Bot-equivalent; default off): `GCS_MIND_SEATS=floor,ops` then `start-studio-bus.sh start` runs `seat-mind-loop.sh` / `mind.py` (mailbox + pin + stay-up; inbox → `grok -p --resume` pinned UUID, `--plugin-dir plugins/studio-mind`; no ACP inject). Mind is the GROW path when opted in; ACP inject is leftover host OS. See `docs/studio/MIND.md`. Do not kill existing serve.
