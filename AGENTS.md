...
- ACP daemons are opt-in: `scripts/a2a/start-studio-bus.sh start --daemons` (serve + inbox wake).
- Inbox wake: `scripts/a2a/wake-daemon.py` → `seat-prompt-acp.sh` → `acp_inject.py --pin-session` (no `grok --resume`).
