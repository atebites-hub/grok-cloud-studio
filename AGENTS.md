...
- Opt-in Grok Build mind (Bot-equivalent; default off): `GCS_MIND_SEATS=floor,ops` then `start-studio-bus.sh start` runs `seat-mind-loop.sh` / `mind.py` (mailbox + pin + stay-up; inbox → `grok --resume` pinned UUID `--prompt-file`, never bare `-p`; `grok plugin install --trust` of `plugins/studio-mind` into seat GROK_HOME; no ACP inject). On grok HTTP 402 / usage balance exhausted (or `GCS_MIND_RUNNER=cursor`), the same mail line may run Cursor CLI (`cursor-grok` or `agent --model cursor-grok-4.6-xhigh`) with a separate `mind/cursor-session` pin; do not remint the grok UUID. Mind is the GROW path when opted in; ACP inject is leftover host OS. See `docs/studio/MIND.md`. Do not kill existing serve.
