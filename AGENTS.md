...
- Ship gate: `.venv/bin/pytest -q` and `python3 scripts/secret_scan.py`. Empty GitHub checks (`check_runs=0`) are not evidence; MERGEABLE is not a substitute. GCS #62 owns the Actions `pull_request` workflow; do not remint it.
