...
- `./doctor.sh` **FAIL**s closed if Cursor Cloud launch-plane is missing (`GCS_CLOUD_REPO` / `CLOUD_REPO_URL`, `CURSOR_API_KEY` boolean-only, `scripts/launch-cloud-extra-high.sh`). Never print the key. Never Bot CloudAgent.
