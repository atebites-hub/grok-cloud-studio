...
- Palemon Linear is Living Sky (linear.app/livingsky, team Livingsky / LIV). NEVER Black Swan Money.
...
- MCP = tools (`plugins/a2a`, `plugins/cursor-cloud`). Seat taskboard stdio MCP plus Linear HTTP catalog live in each isolated `GROK_HOME/config.toml`, never Cursor `${workspaceFolder}`. Cursor Cloud / Cursor CLI: `.cursor/mcp.json` Linear + taskboard only. A2A = seat-to-seat.
