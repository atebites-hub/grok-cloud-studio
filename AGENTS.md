...
- Pin `acp.session`. HANDOFF only after a real start (never 1s silence, never `queue/changed` alone). Stay connected after the first tool. Dead session: one `session/new` after N no-accepts (`ACP_INJECT_SESSION_DEAD`). RESULT is duplex, not success. Do not `session/cancel` a handed-off live turn.
