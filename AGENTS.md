...
- Living Sky Linear: hive stamps `LIV-*` after a real mind turn or Extra High `CLOUD_LAUNCH_OK` (evidence). Hub `TASK_STATE_COMPLETED` is a receipt, not a stamp. Unset `LINEAR_API_KEY` fails closed — do not fake stamps. Never Black Swan. See `docs/studio/LINEAR.md`.
