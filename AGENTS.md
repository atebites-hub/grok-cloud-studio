...
- Pin `acp.session`. HANDOFF only after STATUS (`reason=status`) or session/prompt RPC completion (`reason=rpc-complete`). Never 1s silence, never `queue/changed` alone, never keep-alive acknowledgements. Stay connected after the first tool. Dead session: one `session/new` after N no-accepts (`ACP_INJECT_SESSION_DEAD`). RESULT is duplex, not success. Do not `session/cancel` a handed-off live turn.
