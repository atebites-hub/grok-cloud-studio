...
- Studio Linear is Living Sky (`linear.app/livingsky`, team Livingsky / `LIV`). NEVER Black Swan Money. Grok Build minds stamp `LIV-*` themselves via Linear MCP. Do not have Donald DIY Linear.
...
- MCP = tools (`plugins/a2a`, `plugins/cursor-cloud`). Seat taskboard stdio MCP lives in each isolated `GROK_HOME/config.toml` (`taskboard --db $GCS_TASKBOARD_DB mcp`), never Cursor `${workspaceFolder}`. Linear HTTP is on **both** catalogs: Grok minds via seat `GROK_HOME` (`[mcp_servers.linear]`); Cursor Cloud Extra High via checkout `.cursor/mcp.json` (Linear HTTP + taskboard only; `LINEAR_API_KEY` from cloud-env snapshot / Secrets). Do not copy GROK_HOME MCP into Cursor. Do not have Donald DIY Linear. A2A = seat-to-seat.
