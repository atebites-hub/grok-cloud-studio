...
- Ship gate: `.venv/bin/pytest -q` and `python3 scripts/secret_scan.py`. GitHub Actions `.github/workflows/ship-gate.yml` runs `scripts/ci/ship-gate.sh` on pull requests (N passed required; never `--override-ini`; never Bot CloudAgent). Empty GitHub checks (`check_runs=[]`) are not evidence; MERGEABLE is not a substitute.
