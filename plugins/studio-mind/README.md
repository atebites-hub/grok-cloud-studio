...
`a2a_list_seats`, `cloud_launch`, `cloud_list`, `cloud_status`,
`cloud_followup`, `cloud_result`, plus grok-bot-like `plugins/a2a` and
`plugins/cursor-cloud`. Grok `plugin.json`, not Hermes `plugin.yaml`.
...
uses Cursor builtins plus repo `.cursor/mcp.json` (`gcs-cursor-cloud` plane);
`ticket`, `scripts/a2a/send.sh`, and `scripts/launch-cloud-extra-high.sh` stay
on PATH. Never copy `GROK_HOME` MCP into Cursor CLI.
