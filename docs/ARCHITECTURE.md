...
Common Director footer: `scripts/directors/common_footer.txt`. Every mind turn MUST stamp a Living Sky Linear issue (`LIV-*`). Palemon Linear is Living Sky (`linear.app/livingsky`, team Livingsky / `LIV`). NEVER Black Swan. Stamp it yourself via Linear MCP. Do not have Donald DIY Linear.
