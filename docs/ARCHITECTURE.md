...
send.sh → hub.py (ack + inbox JSONL)
            ↓
        wake-daemon.py → seat-prompt-acp.sh → acp_inject.py --pin-session
                     (live grok agent serve; pin acp.session; no grok --resume)
        dispatch.py  → acp_inject.py (fallback when wake.pid is not live)
