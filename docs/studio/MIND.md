...
| `grok plugin install --trust` | `plugins/studio-mind` into seat `GROK_HOME` from `seat-mind-loop.sh` (`ticket`, `a2a_send`, `cloud_launch`, `cloud_capacity`) |
...
### Capacity beat (LIV-41)

Directors and cloud minds must not treat leftover agent `ACTIVE`+`FINISHED` shells as spinning workers. A capacity beat (`ACP_PING STATUS/CONTINUE`, `CAPACITY_BEAT`) is a **mailbox side-effect** in `mind.py` before the runner thinks: Python **MUST** call `scripts/launch-cloud-extra-high.sh` until latest-run **`runStatus=RUNNING` >= `GCS_CLOUD_MIN_RUNNING`** (default **8**) **per bound remote**. Bound remotes are `GCS_CLOUD_REPOS` (comma-separated), plus `GCS_CLOUD_REPO` / `CLOUD_REPO_URL` and `GCS_GAME_REPO`, counted separately. Count **only** `runStatus=RUNNING`. Never count agent `ACTIVE`. `CREATING` is not `RUNNING`. Unbound agents (no repo URL) are dropped.

Plugin: studio-mind `cloud_capacity`. Same-task retry does not double-fill. Not a second tool-calling loop. Never Bot CloudAgent. Model stays grok-4.6 xhigh, `fast=false`. Living Sky Linear is **LIV**, not Black Swan.
