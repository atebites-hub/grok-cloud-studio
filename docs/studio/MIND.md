...
| `grok plugin install --trust` | `plugins/studio-mind` into seat `GROK_HOME` from `seat-mind-loop.sh` (`ticket`, `a2a_send`, `cloud_launch`, `cloud_list`, `cloud_status`, `cloud_followup`, `cloud_result`) |
