...
- Persist grok json stdout onto `transcript.jsonl`. Bump `offset` only after the effective runner exits 0. That same success marks the hub task `TASK_STATE_COMPLETED`. `send.sh` / hub enqueue is `TASK_STATE_SUBMITTED`. A failed runner leaves mail queued (offset unchanged, task not completed). Empty harvest does not remint and does not mark COMPLETE.
...
Do not consume/advance `offset` unless the effective runner exits 0 (that
success is also when the hub task becomes `TASK_STATE_COMPLETED`). If the
