...
- Persist grok json stdout onto `transcript.jsonl`. Bump `offset` only after the effective runner exits 0. Hub `TASK_STATE_COMPLETED` / A2A ACK is a **receipt, not mind-turn done**. Do not treat send.sh COMPLETE as `MIND_TURN`. Mail is consumed only after grok/cursor runner exit 0. A runner that did not run is not success. Empty harvest does not remint and does not consume mail.
