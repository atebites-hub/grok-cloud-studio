...
That is the grok serve config. Isolated `GROK_HOME` does not inherit `~/.grok/config.toml`. Seat start also registers Linear HTTP catalog (`[mcp_servers.linear] url = "https://mcp.linear.app/mcp"`) in the same file — Grok catalog form, not a copy of `.cursor/mcp.json`. Cursor `${workspaceFolder}` never expands under grok; grok must not load `.cursor/mcp.json`. Seat start sets `[compat.cursor] mcps = false`. `./doctor.sh` WARNs if a seat `config.toml` still contains `${workspaceFolder}`. Refreshing MCP config does not remint a live serve.

Cursor CLI uses a **second catalog**: checkout `.cursor/mcp.json` wrapping `scripts/studio/taskboard/run-mcp.sh` (`taskboard --db $DB mcp`) **and** Linear HTTP (`https://mcp.linear.app/mcp`, `Bearer ${LINEAR_API_KEY}`). Linear + taskboard only. Do not copy `GROK_HOME` MCP into Cursor CLI. Two catalogs. Never fake a transfer. Palemon Linear is Living Sky (linear.app/livingsky, team Livingsky / LIV). NEVER Black Swan Money. No Agent Kanban. No secrets, private GitHub URLs, or MagicDNS hostnames in that file.
