...
advances on runner exit 0, and when skipping a duplicate identical
`FLEET_DONE` line (waiter+shepherd double ping) without a second grok turn.
Huge inboxes rotate the consumed prefix
...
- Grok catalog: seat `GROK_HOME/config.toml` (taskboard stdio
  ...
  usage is back. `SENTRY_DSN` from env / `$GCS_A2A_STATE/sentry.env` only
  (LIV-93).
- Cursor CLI catalog: repo `.cursor/mcp.json` wrapping
  ...
  Extra High Higgsfield is LIV-84 cloud-env snapshot login. Do not
  mcp_auth loop. Do not remint cloud-env.
...
or if `scripts/studio/agent-kanban` reappears
