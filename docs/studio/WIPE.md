...
- Grok catalog: seat `GROK_HOME/config.toml` (taskboard stdio
  ...
  of `plugins/studio-mind`. Palemon Linear is Living Sky
...
`LINEAR_API_KEY` from `$GCS_A2A_STATE/linear.env` (or dashboard Secrets)
so those agents see Linear via `.cursor/mcp.json`.
...
5. Set `CURSOR_API_KEY` in the environment or `~/.config/cursor/agent.env`.
   ...
   Palemon Linear is **Living Sky** (`https://linear.app/livingsky`, team
   Livingsky / `LIV`). **NEVER Black Swan Money.** Write `LINEAR_API_KEY`
   to `$GCS_A2A_STATE/linear.env` (`chmod 600`) or
   `GCS_LINEAR_KEY_FILE`. Never commit that file. Cursor Cloud Extra High
   agents cannot scrape `GROK_HOME`: snapshot `LINEAR_API_KEY` into the
   cloud environment from that secret file (or Cursor dashboard Secrets)
   so `.cursor/mcp.json` Linear works.
