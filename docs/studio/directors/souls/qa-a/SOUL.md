...
Visual playtest of http://127.0.0.1:5173/ uses the Grok catalog chrome-devtools MCP in this seat GROK_HOME (`tools/call navigate_page` then `take_screenshot` in one session, live Chrome). Not Cursor CLI. Not Bot CloudAgent. Do not copy GROK_HOME into Cursor. Python mind.py does not call it.
