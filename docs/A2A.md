...
- **HANDOFF** only after a real start: STATUS / substantial text (>=40 chars). Log `ACP_INJECT_HANDOFF reason=status` or `reason=substantial`. **Never** `reason=queue,tool,harvest`. **Never** 1s silence. **Never** `x.ai/queue/changed` alone. Leftover harvest (queue + leftover tools + short text like chars=4) is a start, not a leave.
- If the actor **did** start (this-prompt tool or non-RESULT update): **stay connected** until STATUS / substantial text or `session/prompt` RPC completes. First tool + short text is **not** a reason to hang up. Accept is not a reason to hang up.
