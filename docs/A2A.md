...
`acp_inject.py` defaults to **180s** (`GCS_ACP_INJECT_TIMEOUT`). Leftover dispatch (no `--pin-session`) harvests a streamed `RESULT` / `PARK_ACK` / `QA_*_RESULT` line, duplexes (`scripts/a2a/duplex.py`), prints `ACP_INJECT_OK`, unlinks `acp.inject.stale`, and best-effort `session/cancel` leftover turn — it does **not** wait for the `session/prompt` JSON-RPC result. If the wait times out but a RESULT line already streamed, that is success, not failure. Timeout/failure with **no** RESULT still sends `session/cancel`, writes `acp.inject.stale` (next inject auto force-new session), and exits non-zero.

`--pin-session` is a live-seat handoff: stay on the websocket after `session/prompt` start. `ACP_INJECT_HANDOFF` fires only on a **this-prompt tool** or a **non-RESULT** `session/update`. It does **not** HANDOFF on 1s of silence, `queue/changed` alone, leftover empty tools, or a RESULT hang-up line. The accept deadline is **30s** (`GCS_ACP_ACCEPT_DEADLINE`). After no-accept, inject remints once (`session/new` on the same socket, `ACP_INJECT_REMINT`) and exits non-zero without `session/cancel`. A true HANDOFF disconnects the inject client and leaves the serve turn running.

Dispatch lock TTL defaults to **240s** (`GCS_DISPATCH_LOCK_TTL_SEC`). If a lock pid is still alive past TTL, dispatch logs `DISPATCH_LOCK_TTL_KILL`, SIGTERM then SIGKILL that pid, clears the lock, and continues the inbox. Inject launches use `--timeout min(inject_timeout, lock_ttl-30)` so inject dies before the TTL killer races it. A mid-turn inject holding the lock for 10–15+ minutes is a bug we kill.
