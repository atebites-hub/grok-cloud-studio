...
scripts/a2a/send.sh ops "ping: hello"           # TASK_STATE_COMPLETED is a receipt, not mind-turn done
...
Hub `TASK_STATE_COMPLETED` / `send.sh` `A2A_SEND_OK` is a protocol **receipt**, not mind-turn done — not Director RESULT and not proof the seat acted. This FAT does not clone LIV-85 mail-hold. `donald` / `orchestrator` stay `skipSeats`.
