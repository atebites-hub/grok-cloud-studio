...
chmod +x "$ROOT/scripts/a2a/bot-bridge.py" "$ROOT/scripts/a2a/bind-bot-agent.sh" \
  ...
  "$ROOT/scripts/directors/mind.py" \
  "$ROOT/scripts/linear_archive_closed.py" \
  "$ROOT/scripts/linear_comment.py" \
  "$ROOT/scripts/linear_stamp.py" \
  "$ROOT/scripts/linear_purge_closed.py" 2>/dev/null || true
...
echo "DR: ./setup.sh (deploy) and ./cleanup.sh (teardown); ./health_check.sh / ./recover.sh; optional scripts/studio/systemd/install-systemd.sh; docs/studio/WIPE.md"
